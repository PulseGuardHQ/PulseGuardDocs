...
curl -H "Authorization: Bearer YOUR_API_TOKEN" \
  https://app.pulseguard.nl/api/v2/domains
```
...
```javascript
const response = await fetch('https://app.pulseguard.nl/api/v2/domains', {
  method: 'POST',
  headers: {
...
response = requests.get(
    'https://app.pulseguard.nl/api/v2/stats/domains/DOMAIN-UUID/response-time/stats',
...
$response = $client->get('https://app.pulseguard.nl/api/v2/domains', [
    'headers' => [
...
#### Register a Device
```bash
curl -X POST https://app.pulseguard.nl/api/v2/devices \
...
const getDeviceMetrics = async (deviceUuid) => {
  const response = await fetch(`https://app.pulseguard.nl/api/v2/devices/${deviceUuid}`, {
    headers: {
      'Authorization': 'Bearer YOUR_API_TOKEN'
    }
  });
...
response = requests.post(
    'https://app.pulseguard.nl/api/v2/services',
    headers={'Authorization': 'Bearer YOUR_API_TOKEN'},
    json=service_data
)
...
```bash
curl -X POST https://app.pulseguard.nl/api/v2/services/SERVICE-UUID/check \
  -H "Authorization: Bearer YOUR_API_TOKEN"
```
...
const dnsLookup = async (domain, recordType = 'A') => {
  const response = await fetch('https://app.pulseguard.nl/api/v2/toolbox/dns-lookup', {
    method: 'POST',
    headers: {
      'Authorization': 'Bearer YOUR_API_TOKEN',
      'Content-Type': 'application/json'
...
response = requests.post(
    'https://app.pulseguard.nl/api/v2/toolbox/ssl-check',
    headers={'Authorization': 'Bearer YOUR_API_TOKEN'},
...
#### Port Scanning
```bash
curl -X POST https://app.pulseguard.nl/api/v2/toolbox/port-scan \
  -H "Authorization: Bearer YOUR_API_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "host": "example.com",
...
async def check_domain_status(session, domain_uuid):
    async with session.get(
        f'https://app.pulseguard.nl/api/v2/domains/{domain_uuid}',
        headers={'Authorization': 'Bearer YOUR_API_TOKEN'}
    ) as response:
...
- **Status Page**: [status.pulseguard.nl](https://status.pulseguard.nl)
- **GitHub Issues**: [Report bugs and feature requests](https://github.com/PulseGuardHQ/feedback)
- **Email Support**: [info@pulseguard.nl](mailto:info@pulseguard.nl)
