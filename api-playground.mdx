...
1. Log into your [PulseGuard Dashboard](https://app.pulseguard.nl)
...
curl -X POST "https://app.pulseguard.nl/api/v2/domains" \
  -H "Authorization: Bearer YOUR_API_TOKEN" \
...
```bash
# Get device metrics
curl -X GET "https://app.pulseguard.nl/api/v2/devices/DEVICE-UUID" \
  -H "Authorization: Bearer YOUR_API_TOKEN"

# Regenerate device token
curl -X POST "https://app.pulseguard.nl/api/v2/devices/DEVICE-UUID/regenerate-token" \
  -H "Authorization: Bearer YOUR_API_TOKEN"
```
...
```bash
# DNS lookup
curl -X POST "https://app.pulseguard.nl/api/v2/toolbox/dns-lookup" \
  -H "Authorization: Bearer YOUR_API_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "domain": "example.com",
...
curl -X POST "https://app.pulseguard.nl/api/v2/toolbox/ssl-check" \
  -H "Authorization: Bearer YOUR_API_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "url": "https://example.com"
...
- Contact support at [info@pulseguard.nl](mailto:info@pulseguard.nl)
